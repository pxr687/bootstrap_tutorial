...
We could also partly define data science as the science of drawing insights from data, 
using code, under uncertainty.

It's that uncertainty that creates what I'll call "The Fundamental Problem" of data science:
...
A random sample is unlikely to *perfectly* capture the population parameter.

It is like to be "off" in either direction.

The code cell below randomly draws a sample from the population we created earlier,
and shows it alongside the population data:

```{python}
# draw a random sample from the population data
population_plot_with_sample(population, sample_size = 20)
```
...
population_plot_with_unrepresentative_sample(population)
```
...
population_plot_sf_data_with_sample(sf2015, bins = sf_bins, sample_size = 20) 
```
